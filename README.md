...
键的写法决定匹配方式，三种写法各管一类文案：

| 键的写法 | 匹配方式 | 例子 |
|---|---|---|
| 键的**第一个和最后一个字符都不是空格** | 整段文案完全一致才替换 | 键 `"Enable Lane Changes"` → 命中这一整条标签 |
| 键的**最后一个字符**是冒号（`:` 或 `：`）或空格 | 只替换文案**开头**的这一段，后面的内容原样保留 | 键 `"Speed:"` → `Speed: 39.7 km/h` 译成 `速度：39.7 km/h` |
| 键的**第一个字符**是空格 | 只替换文案**结尾**的这一段，前面的内容原样保留 | 键 `" vehicles."` → `Sorted 0 vehicles.` 译成 `已排序 0 辆车。` |

后两种写法用于插件把标签与数值拼成一整句的情况。

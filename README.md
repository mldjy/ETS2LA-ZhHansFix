...
| `"By "` | `"作者："` | 原文**结尾**是空格（`By` 后面）→ 只替换开头那一段 |
| `" vehicles."` | `" 辆车。"` | 原文**开头**是空格（引号后第一个字符）→ 只替换结尾那一段 |

#### 空格标记用哪个颜色？（挑一个最明显的）

下面这些方块各自表示「这里是一个空格」，颜色由 emoji 自带，在浅色和深色主题下都能显示。
选一个你看着最清楚的，我把它写进上面的表格：
...
| 🟧 | "By🟧" | 橙，比黄更跳，浅底深底都清楚 |
| 🟥 | "By🟥" | 红，最抢眼 |
| 🟩 | "By🟩" | 绿，对比强但容易和「通过/正确」联想 |
| 🟦 | "By🟦" | 蓝，冷静不刺眼 |
| 🟪 | "By🟪" | 紫，少见，识别度高 |
| 🟫 | "By🟫" | 棕，偏暗，浅底一般 |
| ⬛ | "By⬛" | 黑，浅色主题最明显，深色主题几乎看不见 |
| ⬜ | "By⬜" | 白，需要边框才看得出来 |
